...
function cleanPhrases(str) {
	if (voiceInputSettings.voiceInputDialect.indexOf("en-") != -1) {
...
recognition.onaudiostart = function() {
	...
	setTimeout(function() {
		...
		setTimeout(function() {
			$countdown.text(1);
			setTimeout(function() {
				//$talkButton.find("img").attr("src", "https://apps.jasonsavard.com/images/mic-animate.gif");
		...
		$messageAndFooterAreas.each(function(index, messageAndFooterArea) {
			...
			if ($bottomArea.find(".talkButton").length == 0) {
				...
				$messageAndFooterArea.rehook("keydown", function(e) {
					...
					if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.keyCode == 190) {
						saveLastSelection();
						$talkButton.click();
					}
				});
