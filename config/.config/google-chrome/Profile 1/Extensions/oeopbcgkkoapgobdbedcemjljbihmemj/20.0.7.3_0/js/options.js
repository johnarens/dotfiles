...
function loadLabels(params) {
	...
	if (account) {
		...
		$collapse.append($option);
		$option = generateMonitorLabelOptions(account, getMessage("importantMail") + " " + getMessage("in") + " " + getMessage("inbox"), SYSTEM_IMPORTANT_IN_INBOX, "icons:info-outline");
		$collapse.append($option);
...
function loadAccountsOptions(loadAccountsParams) {
	...
	$.each(allAccounts, function(i, account) {
		...
		var accountObj = {
			...
			showSignatureAndFetchedData: account.getSetting("showSignature", "accountsShowSignature") && account.hasSignature(),
	...
	$("#accountsList")
		...
		.on("click", ".signature", function() {
			var account = getAccountByNode(this);
			var that = this;
			if (account.getSetting("showSignature", "accountsShowSignature") && account.hasSignature()) {
				processEnabledSetting(that, "showSignature");
				showMessage("Signatures disabled");
			} else {
				showLoading();
				account.fetchSendAs().then(sendAsData => {
					if (account.hasSignature()) {
						processEnabledSetting(that, "showSignature");
...
function maybeShowWidgetMenu() {
	...
	if (!$widgetTab.is(":visible")) {
		...
		chrome.runtime.sendMessage("llaficoajjainaijghjlofdfmbjpebpa", {action:"sayHello"}, function(response) {
	...
	polymerPromise.then(function() {
		...
		chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
			if (message.command == "grantPermissionToEmails") {
				...
				var account = getAccountByEmail(message.tokenResponse.userEmail);
				if (!account) {
					console.log("new account");
					// used bg. because localStorage was null inside callback of getChromeWindowOrBackgroundMode
					account = new bg.MailAccount({mailAddress:message.tokenResponse.userEmail});
				...
				account.fetchSendAs().catch(error => {
					...
					account.getEmails().then(() => {
						...
						account.syncSignInId().then(() => {
							...
							if (Settings.read("poll") == "realtime" || Settings.read("poll") <= seconds(30)) {
								alwaysPromise(account.enablePushNotifications()).then(() => {
		...
		if (window.voiceInputLanguage) {
			var voiceInputDialectPref = Settings.read("voiceInputDialect", getPreferredLanguage());
		...
		function initDisplayForAccountAddingMethod() {
			...
			if (Settings.read("accountAddingMethod") == "autoDetect") {
				...
				if (Settings.read("poll") == "realtime") {
					Settings.delete("poll");
				}
			} else {
				$("body").removeClass("autoDetect");
			}
			
			$("#pollingInterval paper-menu")[0].select( calculatePollingInterval(bg.accounts) );
		}
		...
		$("#accountAddingMethod paper-radio-button").click(function() {
			...
			alwaysPromise(pollAndLoad({showNotification:false, refresh:true})).then(() => {
				initDisplayForAccountAddingMethod();
				bg.restartCheckEmailTimer(true);
			});
		});
		
		$("#pollingInterval paper-menu").on("iron-activate", function(e) {
			...
			if (pollingInterval == "realtime" && !DetectClient.isChrome()) {
			...
			} else if (Settings.read("accountAddingMethod") == "autoDetect" && pollingInterval == "realtime") {
				e.preventDefault();
				$("#accountAddingMethod").addClass("highlight");
				setTimeout(function() {
					$("#accountAddingMethod").removeClass("highlight");
				}, seconds(3));
				niceAlert(getMessage("onlyAvailableWhenUsingX", "<b>" + getMessage("addAccountsStaySignedIn") + "</b>"));
			} else {
				var previousPollingInterval = Settings.read("poll");
				...
				new Promise((resolve, reject) => {
					...
					Settings.store("poll", pollingInterval);
					bg.restartCheckEmailTimer(true);
				}).catch(error => {
					console.error(error)
					$("#pollingInterval paper-menu")[0].select( previousPollingInterval );
