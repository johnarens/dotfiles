	...
	if (Settings.read("usedMarkAllAsReadButton")) {
		if (account.unreadCount > MAX_EMAILS_TO_ACTION) {
			...
			openDialog($dialog).then(response => {
				if (response == "ok") {
					showLoading();
					bg.markAllAsX($account, account, action, closeWindow).then(() => {
						refresh();
					}).catch(error => {
						showError(error);
					});
					...
					$account.find(".colors")
						.click(function() {
							...
							if (donationClicked("colors")) {
								var $paperColorPicker = $("paper-color-picker");
								$paperColorPicker.off().remove();
								//if (!$paperColorPicker.length) {
									$paperColorPicker = $("<paper-color-picker always-show-alpha allow-alpha></paper-color-picker>");
									$("body").append($paperColorPicker);
									//Polymer.dom($("body")[0]).appendChild($paperColorPicker[0]);
								//}
								
								// timeout patch for Firefox because paper-color-picker was not rendering
								setTimeout(function() {
									$paperColorPicker[0].open();

									var colorString = account.getSetting("accountColor");
									//account.deleteSetting("accountColor");
									if (/rgba/.test(colorString)) {
										// "rgba(111,222,333,0.5)"
									    var colorsOnly = colorString.substring(colorString.indexOf('(') + 1, colorString.lastIndexOf(')')).split(/,\s*/);
									    var immediateColor = {};
									    immediateColor.red = colorsOnly[0];
									    immediateColor.green = colorsOnly[1];
									    immediateColor.blue = colorsOnly[2];
									    immediateColor.alpha = colorsOnly[3];
										$paperColorPicker[0].immediateColor = immediateColor;
									}

									// Must set this AFTER opening paperColorPicker.open()
									$paperColorPicker.on("immediate-color-changed", function(e) {
										/*
										var rgba = e.originalEvent.detail.value;
										console.log("START path", e.originalEvent.detail.path);
										console.log("immediate-color-changed", e.originalEvent);
										console.log('rgba(' + $paperColorPicker[0].immediateColor['red'] + ',' + $paperColorPicker[0].immediateColor['green'] + ',' + $paperColorPicker[0].immediateColor['blue'] + ',' + $paperColorPicker[0].immediateColor['alpha'] + ')');
										console.log('rgba(' + rgba.red + ',' + rgba.green + ',' + rgba.blue + ',' + rgba.alpha != undefined ? rgba.alpha : 1 + ')');
										console.log("detail", e.originalEvent.detail.value);
										 */
										
										if (e.originalEvent.detail.path) {
											updateAccountHeaderColor(account, $account, 'rgba(' + $paperColorPicker[0].immediateColor['red'] + ',' + $paperColorPicker[0].immediateColor['green'] + ',' + $paperColorPicker[0].immediateColor['blue'] + ',' + $paperColorPicker[0].immediateColor['alpha'] + ')');
										}
										
										//updateAccountHeaderColor(account, $account, 'rgba(' + rgba.red + ',' + rgba.green + ',' + rgba.blue + ',' + (rgba.alpha != undefined ? rgba.alpha : 1) + ')');
									});
...
function renderMails(params) {
	...
	mails.some(function(mail, mailIndex) {
		...
		if ($lastMail.length && !isVisibleInScrollArea($lastMail, $("[main]"), existingMailsCount + mailIndex)) {
			mailNodesBelowFold++;
		}
...
function getInboxViewportHeight() {
	// used to be $(window) but firefox gave me different results??
	return $("body").height() - getInboxTop() - 4;
}

function resizeInboxNode() {
	// need to wait for polymer to load to calculate heights
	polymerPromise.then(function() {
		// timeout required for Firefox or else $("#inbox").offset().top returns 0 because it's not rendered i guess
		setTimeout(function() {
...
function resizeNodes() {
	...
	if (isDetached) {
		if (popupView == POPUP_VIEW_CHECKER_PLUS) {
			...
			$("#tabletViewFrame").height( $("body").height() -  $("#tabletViewFrame").offset().top - 10 );
		}
	}
}
...
function init() {
	...
	*/
	...
	return bgObjectsReady.then(() => {
		...
		// In Chrome 55 the popup was delayed from rendering when ANY <link> tags were found (css or import)
		/*
		pageVisible.then(() => {
			insertImport("extensionCommon.html");
		});
		*/
		...
		$("#maximize").mousedown(function(e) {
			if (isCtrlPressed(e)) {
				//bg.openInPopup();
				openWindowInCenter(chrome.runtime.getURL("materialDesign.html"), '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes', Settings.read("popupWidth"), Settings.read("popupHeight"));
			...
			$(".dnd")
				.click(function() {
					...
					$dialog.find("#dndInFullscreen").off().on("change", function() {
						Settings.store("dndInFullscreen", this.checked).catch(error => {
							showError(error);
						});
					});
					...
					$dialog.find("#showGrayIconInDND").off().on("change", function() {
						Settings.store("showGrayIconInDND", this.checked).catch(error => {
							showError(error);
						});
					});
