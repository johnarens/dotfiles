...
function MailAccount(accountParams) {
   ...
   function fetchFeed(params) {
	   return new Promise((resolve, reject) => {
		   ...
		   var useCache = (label == LABEL_TO_ENSURE_MAIL_ADDRESS_IS_IDENTIFIED) && cachedInboxFeedRequest;
		   var jqxhr;
		   if (useCache) {
			   console.info("Use inbox feed cache");
   ...
   function fetchEmailsByLabel(params) {
	   ...
	   if (params.monitorLabelsIndex >= params.monitorLabels.length) {
		   ...
		   return fetchFeed(params).then(fetchFeedResponse => {
   ...
   this.hasMonitoredLabel = function(labelId) {
	   return that.getMonitorLabels().some(monitorLabel => {
   ...
   this.enablePushNotifications = function() {
	   ...
	   return ensureGCMRegistration().then(registrationId => {
		   ...
		   return ajax({type:"post", url:"https://cool-kit-794.appspot.com/ajax", contentType:"application/json", data:JSON.stringify(data)}).then(() => {
			   // Start watching
			   return that.watch();
		   });
	   }).catch(error => {
   ...
   this.disablePushNotifications = function() {
	   return bg.oAuthForEmails.send({userEmail:mailAddress, type:"post", url: GmailAPI.URL + "stop"});
   }
   ...
   this.watch = function() {
	   ...
	   return bg.oAuthForEmails.send({userEmail:mailAddress, type:"post", url: GmailAPI.URL + "watch", data:data, noCache:true}).then(response => {
		   ...
		   that.saveSetting("watchExpiration", watchExpiration);
   ...
   this.startWatchAlarm = function() {
	   var DELAY_BETWEEN_STOP_AND_START_IN_SECONDS = 5;
	   var watchExpiration = that.getSetting("watchExpiration");
	   if (watchExpiration) {
   ...
   this.isBeingWatched = function() {
	   return that.getSetting("watchExpiration") && that.getSetting("watchExpiration").isAfter();
   }
   ...
   this.getError = function(useHtml, document) {
	   ...
	   if (Settings.read("accountAddingMethod") == "autoDetect") {
		   if (that.errorCode == JError.GOOGLE_ACCOUNT_WITHOUT_GMAIL) {
			   ...
			   if (useHtml) {
				   $instructions.append("Use the " , createButton("options.html?ref=cannotEnsureMainAndInbox#accounts", "addAccount"), "option instead!");
			   } else {
   ...
   this.getEmails = function() {
	   ...
	   return new Promise((resolve, reject) => {
		   if (Settings.read("accountAddingMethod") == "autoDetect") {
			   fetchEmailsByLabel({monitorLabels:that.getMonitorLabels(), monitorLabelsIndex:0, feedsArrayInfo:[]}).then(fetchEmailsResponse => {
				   var inboxFeed;
			   ...
			   function getInitialMessages(params) {
				   return new Promise((resolve, reject) => {
					   ...
					   getMessagesByList({monitoredLabels:params.account.getMonitorLabels(), historyId:params.historyId}).then(newestMailArray => {
					   ...
					   getMessagesByHistory(getMessagesByHistoryParams).then(messagesByHistoryResponse => {
						   return new Promise((resolve, reject) => {
							   ...
							   if (histories && histories.length) {
								   ...
								   fetchMessagesByIds(messagesToFetch).then(fetchMessagesByIdsResponse => {
   ...
   function getHistoryForFirstTime() {
	   return new Promise(function(resolve, reject) {
		   bg.oAuthForEmails.send({userEmail:mailAddress, url: GmailAPI.URL + "profile", noCache:true}).then(response => {
	   ...
	   return new Promise((resolve, reject) => {
		   ...
		   if (params.action == MailAction.MARK_AS_READ) {
			   ...
			   if (replyObj && replyObj.from) {
				   ...
				   var tos = params.mail.messages.last().to;
				   if (tos.length == 1 && /@/.test(tos[0].email)) {
					   fromObj.email = tos[0].email;
				   } else {
					   var ccs = params.mail.messages.last().cc;
					   if (ccs.length == 1 && /@/.test(ccs[0].email)) {
						   fromObj.email = ccs[0].email;
					   } else {
			   ...
			   if (params.mail.account) {
				   var profileInfo = params.mail.account.getSetting("profileInfo");
				   if (profileInfo) {
			   ...
			   if (replyObj) {
				   replyObj.tos.forEach(function(to, index) {
					   tos.push(convertAddress(to, true));
				   });
			   } else {
				   tos.push(convertAddress(params.to, true));
			   }
			   
			   var ccs = [];
			   if (replyObj && replyObj.ccs && replyObj.ccs.length) {
				   replyObj.ccs.forEach(function(cc, index) {
					   ccs.push(convertAddress(cc, true));
				   });
			   }
			   
			   var subject = params.mail.title;
			   
			   var MIN_BODY_LENGTH = 7;
			   var body = params.message;
			   if (body != "") {
				   for (var a=0; body.length < MIN_BODY_LENGTH; a++) {
					   body += " ";
				   }
			   }
			   ...
			   ajaxParams.method = "POST";
			   ajaxParams.data = "=";
			   ajaxParams.data += "&from=" + encodeURIComponent(fromObj.email);
			   tos.forEach(to => {
				   ajaxParams.data += "&to=" + encodeURIComponent(to);
			   });
			   ccs.forEach(cc => {
				   ajaxParams.data += "&cc=" + encodeURIComponent(cc);
			   });
			   ajaxParams.data += "&subject=" + encodeURIComponent("Re: " + subject);
			   ajaxParams.data += "&rm=" + params.mail.id;
			   ajaxParams.data += "&cans=&ctok=&ac=%5B%5D&adc=&ishtml=0&nowrap=0&body=" + encodeURIComponent(body) + "&pte=&pti=&bpfs=&uet=" + encodeURIComponent("\n\n> " + params.mail.getLastMessageText() + "\n\n") + "&pbgt=&pbgas=&pbgir=&msg_encrypt=0&signature=1&securityindicator=0";
			   */
		   } else {
		   ...
		   if (params.action == MailAction.DELETE || params.action == MailAction.MARK_AS_READ || params.action == MailAction.ARCHIVE) {
			   if (params.mail.account.getSetting("gmailSettings").conversationViewMode === false && Settings.firstTime("conversationViewWarningShown")) {
   ...
   function executeGmailAPIAction(params) {
	   return new Promise((resolve, reject) => {
		   ...
		   */
		   ...
		   var sendParams = {userEmail:mailAddress, type:"POST", url:url, timeout:params.timeout};
		   if (params.data) {
   ...
   function generateExecuteGmailAPIActionParams(params) {
	   ...
	   if (account.getSetting("showSignature", "accountsShowSignature")) {
		   var sendAsData = account.getSetting("sendAsData");
		   if (sendAsData) {
			   var signature;
			   ...
			   if (signature) {
				   ...
				   params.htmlMessage += '<br><br clear="all"><div><br></div>-- <br><div class="gmail_signature" data-smartmail="gmail_signature">' + signature + '</div>'; 
			   }
		   }
	   }
   ...
   this.getSetting = function(attributeName, settingsName) {
	   ...
	   if (attributeName == "openLinksToInboxByGmail" && that.getSetting("gmailSettings").inboxByGmail) {
		   return true;
	   } else {
		   return Settings.defaults[settingsName];
	   }
   }
   ...
   this.saveSetting = function(key, value) {
	   ...
	   return Settings.store("emailSettings", emailSettings);
   }
   ...
   this.isUsingGmailCategories = function() {
		var gmailSettings = that.getSetting("gmailSettings");
		return that.getSetting("openLinksToInboxByGmail") || gmailSettings.inboxByGmail || (gmailSettings.tabs && gmailSettings.tabs.length >= 2);
   }
   
   this.isMaybeUsingGmailCategories = function() {
		var gmailSettings = that.getSetting("gmailSettings");
		return that.getSetting("openLinksToInboxByGmail") || gmailSettings.inboxByGmail == undefined || gmailSettings.inboxByGmail || gmailSettings.tabs == undefined || (gmailSettings.tabs && gmailSettings.tabs.length >= 2);
   }

   this.hasHiddenTabs = function() {
		var gmailSettings = that.getSetting("gmailSettings");
		return that.isUsingGmailCategories() && (gmailSettings.tabs && gmailSettings.tabs.length <= 4); // 5 means ok because they are all shown
   }

   this.getMonitorLabels = function() {
	   var monitorLabels = that.getSetting("monitorLabel", "monitorLabelsForGmailClassic");
	   /*
	   if (ltVersion(Settings.read("installVersion"), "19.2.17")) {
		   monitorLabels = that.getSetting("monitorLabel", "monitorLabelsForGmailClassic");
	   } else {
		   if (that.isUsingGmailCategories()) {
			   monitorLabels = that.getSetting("monitorLabel", "monitorLabelsForGmailCategories");
		   } else {
			   monitorLabels = that.getSetting("monitorLabel", "monitorLabelsForGmailClassic");
		   }
	   }*/
   ...
   this.getEmailDisplayName = function() {
	...
	this.getMailUrl = function (params) {
		...
		if (params.detectInboxByGmail && that.getSetting("openLinksToInboxByGmail") && !params.useStandardGmailUrl) {
		...
		if (params.useGmailUI && !usingInboxByGmail) {
			...
			if (messageId) {
				...
				} else if (that.getSetting("conversationView") && threadId) {
					mailUrl += "/" + threadId;
				} else {
					mailUrl += "/" + messageId;
				}
   ...
   this.fetchGmailSettings = function(params) {
	   ...
	   return that.ensureMailAddressIdentified().then(() => {
		   return new Promise((resolve, reject) => {
			   // fetch every 5 days
			   var lastFetchedGmailSettings = that.getSetting("lastFetchedGmailSettings");
			   if (lastFetchedGmailSettings && lastFetchedGmailSettings.diffInDays() > -5) { // !params.refresh && 
				   resolve();
			   } else {
				   ...
				   var jqxhr = $.ajax({
					   ...
					   if (data) {
						   ...
						   var inboxByGmail = jqXHR.responseURL && jqXHR.responseURL.indexOf("inbox.google.com") != -1;
						   
						   that.saveSetting("gmailSettings", {conversationViewMode:conversationViewMode, tabs:tabs, inboxByGmail:inboxByGmail});
						   resolve();
					   } else {
						   reject(textStatus);
					   }
   ...
   function fetchLabelsFromHtmlSource() {
	   ...
	   return ajax({
		   ...
		   logError("An error occured while fetching globals: " + jqXHR.statusText, jqXHR);
   ...
   this.fetchSendAs = function() {
		return bg.oAuthForEmails.send({userEmail:that.getAddress(), url: GmailAPI.URL + "settings/sendAs"}).then(response => {
			var sendAsData = response.data
			that.saveSetting("sendAsData", sendAsData);
			return sendAsData;
		});
   }
   
   this.hasSignature = function() {
		var sendAsData = that.getSetting("sendAsData");
		if (sendAsData) {
		   ...
		   if (that.messages) {
			   ...
			   if (that.deliveredTo && that.deliveredTo.length) {
				   ...
				   if (that.account.getAddress() == replyObj.from.email) {
					   var profileInfo = that.account.getSetting("profileInfo");
