...
String.prototype.htmlToText = function() {
	...
	*/
	...
	return $("<textarea/>").html(this).text().replace(/<br\s?\/?>/ig,"\n").replace(/<(?:.|\n)*?>/gm, '');
}
...
function setStorage(element, params) {
	...
	if (($(element).closest("[mustDonate]").length || params.mustDonate) && !Settings.read("donationClicked") && !OFF_OR_DEFAULT) {
		...
		if (params.account) {
			...
			return Settings.store(params.key, params.value);
		}
	}
}
...
function initConsole() {
	if (bg.Settings.read("console_messages")) {
		/*
...
if (typeof($) != "undefined") {
	if (location.href.indexOf("popup.") != -1 || location.href.indexOf("options.") != -1 || location.href.indexOf("materialDesign.") != -1) {
						...
						chrome.i18n.detectLanguage(chromeTTSMessage.utterance, result => {
							chrome.tts.getVoices(voices => {
								...
								chrome.tts.speak(chromeTTSMessage.utterance, {
									...
									//enqueue : true,
									volume: Settings.read("voiceSoundVolume") / 100,
									pitch: parseFloat(Settings.read("pitch")),
									rate: parseFloat(Settings.read("rate")),
	...
	this.execute = function(sendOAuthParams) {
		return new Promise((resolve, reject) => {
			...
			if (httpRequests.length) {
				...
				sendOAuthParams.oauthRequest.send({tokenResponse:tokenResponse, type:"POST", url: GmailAPI.DOMAIN + "/batch", contentType:"multipart/mixed", boundary:boundary, headers:{"Content-Length":totalContentLength}, processData:false, data:data}).then(sendResponse => {
...
function OAuthForDevices(params) {
	...
	var securityToken = getUniqueId();
	
	var state = chrome.runtime.getURL("oauth2callback.html?security_token=" + securityToken);
	...
	function onTokenErrorWrapper(tokenResponse, response) {
		...
		if ((response.oauthAction == "refreshToken" && (response.jqXHR && (response.jqXHR.status == 400) || response.jqXHR.status == 401))) {
	...
	this.openPermissionWindow = function(email) {
		return new Promise((resolve, reject) => {
			var url = GOOGLE_AUTH_URL + "?response_type=code&client_id=" + GOOGLE_CLIENT_ID + "&redirect_uri=" + GOOGLE_REDIRECT_URI + "&scope=" + encodeURIComponent(that.oauthParams.scope) + "&state=" + encodeURIComponent(that.getStateParam());
	...
	function refreshToken(tokenResponse) {
		...
		var data = {refresh_token:tokenResponse.refresh_token, client_id:GOOGLE_CLIENT_ID, client_secret:GOOGLE_CLIENT_SECRET, grant_type:"refresh_token"};
	...
	this.getAccessToken = function(params) {
		...
		return ajax({
			type: "POST",
			url: GOOGLE_TOKEN_URL,
			data: {code:params.code, client_id:GOOGLE_CLIENT_ID, client_secret:GOOGLE_CLIENT_SECRET, redirect_uri:GOOGLE_REDIRECT_URI, grant_type:"authorization_code"},
			dataType: "json",
			timeout: 8000
		}).then(data => {
			var tokenResponse = data;
			
			return that.getUserEmail(tokenResponse, sendOAuthRequest).then(response => {
...
function passedRandomTime(name, maxDaysFromNow) {
	var randomTime = Settings.read(name);
	...
	if (randomTime) {
		...
		Settings.store(name, randomTime);
		return false;
	}
}
...
function countEvent(eventName) {
	...
	var countEvent;
	var countEventStr = bg.Settings.read(lsKey);
	...
	bg.Settings.store(lsKey, countEvent);
}
...
var Settings = function(params) {
   ...
   Settings.defaultsForOauth = params.defaultsForOauth;
   Settings.extraFeatures = params.extraFeatures;
   ...
   Settings.read = function(key) {
	   var defaultForOauth = this.defaultsForOauth[key];
	   
	   if (cache[key] != null) {
		   return cache[key];
	   } else if (defaultForOauth != undefined && Settings.read("accountAddingMethod") == "oauth") {
		   return defaultForOauth;
...
function ensureGCMRegistration() {
	return new Promise((resolve, reject) => {
		if (Settings.read("registrationId")) {
			console.log("reusing gcm regid");
			resolve(Settings.read("registrationId"));
		} else {
			if (chrome.gcm) {
				chrome.gcm.register([GCM_SENDER_ID], registrationId => {
					console.log("register gcm");
					if (registrationId) {
						console.log("registrationId", registrationId);
						Settings.store("registrationId", registrationId);
