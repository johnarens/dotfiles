...
function initOauthAccounts() {
	...
	if (bg.accounts) {
		bg.accounts.forEach((account, i) => {
			...
			// legacy: migrate users to push notifications and maybe ensure watch/timer
			if (Settings.firstTime("migrateUserToPushNotifications")) {
				// note that if users purposely selected 30sec this will still enable push notification, probably not what we want
				if ((Settings.read("poll") == "realtime" || Settings.read("poll") <= seconds(30)) && !bg.accounts[i].isBeingWatched()) {
					var pushNotificationPromise = bg.accounts[i].enablePushNotifications();
