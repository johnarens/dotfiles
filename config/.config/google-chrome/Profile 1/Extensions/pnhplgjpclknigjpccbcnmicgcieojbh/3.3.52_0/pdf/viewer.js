                    ...
                    if (index == (quadRects.length - 1) && data.comments.length) {
                        ...
                        $(menu).hover(function (e) {
                            ...
                            note.css({
                                ...
                                top: menu_top + 'px',
                                position: "absolute"
                            }).draggable({handle: '.diigo-note-header', opacity: 0.8}).fadeIn('fast');
                        }, function (e) {
                            var self = this;
                    ...
                    $(rect_div).hover(function (e) {
                        ...
                        if (!$('#diigolet-annMenu').hasClass("show") || $('#diigolet-annMenu').attr('data-diigo-id') != ann_id) {
                            ...
                            window.setTimeout(function () {
                                ...
                                $('#diigolet-annMenu').attr('data-page-index', pageIndex).attr('data-diigo-id', ann_id).attr('data-diigo-color', color).css({
                                    left: left + 'px',
                                    top: e.pageY - 55 + 'px'
                                }).show();
                            }, 300);
        ...
        drawDiigoFloatnotes: function drawDiigoFloatnotes() {
            ...
            for (i = 0, ii = floatnotes.length; i < ii; i++) {
                ...
                $(element).hover(function () {
                    ...
                    note.attr('data-diigo-id', $(this).attr('data-diigo-id')).attr('data-origin', $(this).attr('data-origin')).attr('diigo-note-type', 'floatnote').removeClass('yellow blue green pink').addClass($(this).attr('data-diigo-color')).css({
...
var GOOGLE_DRIVE_FILE_URL = "https://content.googleapis.com/drive/v2/files",
    GOOGLE_PROFILE_URL = "https://content.googleapis.com/plus/v1/people/me";
...
function _getFileList() {
    return _requestWithOauth(GOOGLE_DRIVE_FILE_URL, {q: 'mimeType="application/pdf"'});
}

function _getGoogleUserInfo() {
    return _requestWithOauth(GOOGLE_PROFILE_URL, {}).then(function (info) {
        return _getUserPhoto(info.image.url, info);
    });
}
...
function _getGdriveDataList() {
    return _requestWithOauth(GOOGLE_DRIVE_FILE_URL, {q: '"appdata" in parents'});
}
...
function buildFilePicker() {
    initFilePicker().then(function (info) {
        console.log(info);
        var email = info[0].emails[0].value,
            displayName = info[0].displayName,
            items = info[1].items,
        ...
        if (items.length) {
            items.forEach(function (item, index, arr) {
                ...
                $item.on('click', function (e) {
                    ...
                    $('#google-select').data('url', $(this).attr('data-url')).data('title', $(this).attr('data-title')).removeClass('disabled');
    ...
    function uploadAndShare() {
        PDFView.pdfDocument.getData().then(
            function (data) {
                ...
                Web.getPDFSharableLink(obj).then(function (a, b) {
                    console.log('D:', a, b);
                }).catch(function (err) {
                    console.error(err);
                });
